#r "nuget: OptimizedPriorityQueue, 5.0.0"
...
void Solve(State start)
{
	...
	IPriorityQueue<(State, (long, long), long), long> toTest = new SimplePriorityQueue<(State, (long, long), long), long>();
	toTest.Enqueue((start, (0, 0), 0), 0);

	while (toTest.Count > 0)
	{
		var (from, (x, y), steps) = toTest.Dequeue();
		for (long d = 1; d <= 4; d++)
		{
			long output = -1;
			if (from.StepTo(() => d, o => output = o, 4, out State to) && output > 0)
			{
				...
				if (visited.Add(nextPos))
				{
					...
					toTest.Enqueue((to, nextPos, steps + 1), steps + 1);
				}
			}
		}
	}
...
Solve(State.Parse(File.ReadAllText("input.txt")));