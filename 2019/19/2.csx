...
HashSet<(long x, long y)> corners = new HashSet<(long, long)>();

long previousStart = 0;
for (long y = 0; true; y++)
{
	HasFound hasFound = HasFound.NotYet;
	for (long x = previousStart; hasFound == HasFound.NotYet ? x < previousStart + 3 : hasFound != HasFound.Lost; x++)
	{
		bool isX = true;
		...
		if (isAffected)
		{
			if (hasFound == HasFound.NotYet)
			{
				if (corners.Contains((x, y)))
				{
					WriteLine();
					WriteLine(x * 10000 + y - (SearchSize - 1));
					return;
				}
				previousStart = x;
				hasFound = HasFound.RightNow;
			}
		}
		else if (hasFound == HasFound.RightNow)
		{
			if (x >= SearchSize)
			{
				corners.Add((x - SearchSize, y + (SearchSize - 1)));
			}
			hasFound = HasFound.Lost;
		}
	}
}
