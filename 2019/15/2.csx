...
var start = State.Parse(File.ReadAllText("input.txt"));
ISet<(long, long)> visited = new HashSet<(long, long)>();
IList<(long, long)> oxygen = new List<(long, long)>();
Queue<(State, (long, long))> toTest = new Queue<(State, (long, long))>();
toTest.Enqueue((start, (0, 0)));

while (toTest.Count > 0)
{
	var (from, (x, y)) = toTest.Dequeue();
	for (long d = 1; d <= 4; d++)
	{
		long output = -1;
		if (from.StepTo(() => d, o => output = o, 4, out State to) && output > 0)
		{
			var nextPos = (x + (d switch { 3 => -1, 4 => 1, _ => 0 }), y + (d switch { 1 => -1, 2 => 1, _ => 0 }));
...
long steps = -1;
IList<(long, long)> nextOxygen = new List<(long, long)>();
while (visited.Count > 0)
{
	foreach (var pos in oxygen)
	{
		visited.Remove(pos);
	}
	foreach (var (x, y) in oxygen)
	{
	...
	(oxygen, nextOxygen) = (nextOxygen, oxygen);
	nextOxygen.Clear();
	steps++;
}
WriteLine(steps);