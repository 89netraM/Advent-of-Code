...
public class Day25
{
	...
	public object Part1(string input)
	{
		var map = input.Lines()
			.Select(l => l.Split(": "))
			.ToDictionary(s => s[0], s => s[1].Words().ToHashSet());
		foreach (var (from, tos) in map.ToArray())
		{
			foreach (var to in tos)
			{
				if (!map.TryGetValue(to, out var toFrom))
				{
					toFrom = new HashSet<string>();
					map[to] = toFrom;
				}
				toFrom.Add(from);
			}
		...
		var usages = new Dictionary<UOP<string>, long>();
		var nodes = map.Keys.ToArray();
		for (int i = 0; i < nodes.Length; i++)
		{
			BFS.SearchWithPath(
				...
				p =>
				{
					foreach (var (from, to) in p.Zip(p.Skip(1)))
					{
						usages.Increase(UOP.New(from, to));
					}
				});
		}

		foreach (var ((a, b), _) in usages.OrderByDescending(kvp => kvp.Value).Take(3))
		{
			map[a].Remove(b);
			map[b].Remove(a);
		}
		...
		return (nodes.Length - visited) * visited;
	}
}
