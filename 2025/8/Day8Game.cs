...
public class Day8Game : Game
{
	...
	private readonly IImmutableList<IImmutableList<int>> circuits;
	private int attachedLightStrings = 0;
	private int litCircuits = 0;
	...
	public Day8Game(
		...
		IImmutableList<IImmutableList<int>> circuits
	)
	{
		graphics = new(this);
	...
	protected override void Update(GameTime gameTime)
	{
		...
		if (IsMouseClick(gameTime, mouseState))
		{
			...
			else if (litCircuits < circuits.Count)
			{
				foreach (var circuitLightString in circuits[litCircuits])
				{
					lightStrings[circuitLightString].IsLit = true;
				}
				litCircuits++;
			}
		}
