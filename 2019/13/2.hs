...
import qualified Data.List as DL (find)
import Data.List.Split (splitOn)
import Data.Map (elems, Map, empty, insert, lookup, (!), toList)
...
initArcadeS :: ArcadeS
initArcadeS = ArcadeS (0, 0) X 0 empty
interpretWithArcade :: Interpreter a -> Integer
interpretWithArcade i = score m
  where
    (_, m) = SM.runState (interpretWith get' put' i) initArcadeS
    get' = do
      s <- SM.get
      return $ if elem '▒' $ elems $ tiles s
        then do
          let bX = xOf '*' $ tiles s
          let pX = xOf '-' $ tiles s
          if pX < bX
    ...
    put' i = do
      ...
      case (action s, pos s) of
        ...
        (Tile, (-1, 0)) -> SM.put s{ score = i, action = X }
        (Tile, p) -> do
          let ts = tiles s
              c = case i of
                0 -> ' '
    ...
    xOf c m = fst $ fst $ fromJust $ DL.find (\(_, c') -> c == c') $ toList m

main :: IO ()
main = do
    p <- readProgram "input.txt"
    let p' = insert 0 2 p
    let s = interpretWithArcade (executeProgram (State p' 0))
    print s