...
string[] chunks = File.ReadAllText("input.txt").Trim().Split("\n\n");

HashSet<int>[] opcodes = Enumerable.Range(0, 16).Select(_ => new HashSet<int>(Enumerable.Range(0, 16))).ToArray();
...
foreach (string chunk in chunks)
{
	...
	if (lines.Length == 3 && lines[0].StartsWith("Before:") && lines[2].StartsWith("After:"))
	{
		...
		HashSet<int> possibleInstructions = new HashSet<int>();
		for (int i = 0; i < instructions.Length; i++)
		{
			if (instructions[i](a, b, c, before) == after)
			{
				possibleInstructions.Add(i);
			}
		}
		opcodes[opcode].IntersectWith(possibleInstructions);
	}
}

while (opcodes.Any(static p => p.Count > 1))
{
	foreach (HashSet<int> possibleInstructions in opcodes.Where(static p => p.Count == 1))
	{
		foreach (HashSet<int> opcode in opcodes.Where(static p => p.Count > 1))
		{
			opcode.ExceptWith(possibleInstructions);
		}
	}
}

Register result = new Register(0, 0, 0, 0);
foreach (string line in chunks[^1].Split('\n'))
{
	...
	int b = Int32.Parse(parts[2]);
	int c = Int32.Parse(parts[3]);
	result = instructions[opcodes[opcode].Single()](a, b, c, result);
}
WriteLine(result.one);
