...
static int Units(Group g) =>
	g.units;
static bool IsAlive(Group g) =>
	g.units > 0;

string input = File.ReadAllText("input.txt");
string[] sides = input.Split("\n\n");

List<Group> ogImmuneSystem = sides[0].Split('\n').Skip(1).Where(static l => !String.IsNullOrWhiteSpace(l)).Select(Group.Parse).ToList();
List<Group> ogInfection = sides[1].Split('\n').Skip(1).Where(static l => !String.IsNullOrWhiteSpace(l)).Select(Group.Parse).ToList();

int minBoost = 1;
int maxBoost = 1000;
while (true)
{
	int boost = (minBoost + maxBoost) / 2;
	List<Group> immuneSystem = ogImmuneSystem.Select(g => new Group(g.units, g.hp, g.weaknesses, g.immunities, g.attack + boost, g.attackType, g.initiative)).ToList();
	List<Group> infection = ogInfection.Select(static g => new Group(g.units, g.hp, g.weaknesses, g.immunities, g.attack, g.attackType, g.initiative)).ToList();
	while (immuneSystem.Sum(Units) > 0 && infection.Sum(Units) > 0)
	{
		...
		foreach (KeyValuePair<Group, Group> kvp in targets.OrderByDescending(static kvp => kvp.Value.initiative))
		{
			...
			{
				int unitsBefore = kvp.Key.units;
				kvp.Key.Damage(kvp.Value);
				if (kvp.Key.units != unitsBefore)
				{
					didDamage = true;
				}
				if (kvp.Key.units == 0)
	...
	{
		WriteLine(immuneSystem.Sum(Units));
		return;
	}
	else if (infection.Sum(Units) > 0)
	{
		minBoost = Math.Max(minBoost + 1, boost);
	}
	else
	{
		maxBoost = boost;
